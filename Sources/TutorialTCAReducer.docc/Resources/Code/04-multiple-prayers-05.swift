import ComposableArchitecture

@Reducer
struct ParentFeature {
    @ObservableState
    struct State: Equatable {
        @Presents var profile: ProfileFeature.State?
        @Presents var settings: SettingsFeature.State?
        @Presents var workout: WorkoutFeature.State?
        var errorMessage: String?
    }
    
    @Dependency(\.analytics) var analytics
    @Dependency(\.cloudSync) var cloudSync
    
    enum Action {
        case handleError(String)
        case trackAnalytics(String)
        case syncToCloud
        case profile(PresentationAction<ProfileFeature.Action>)
        case settings(PresentationAction<SettingsFeature.Action>)
        case workout(PresentationAction<WorkoutFeature.Action>)
    }
    
    var body: some ReducerOf<Self> {
        Reduce { state, action in
            switch action {
            case let .handleError(message):
                state.errorMessage = message
                return .none
                
            case let .trackAnalytics(eventName):
                return .run { _ in
                    await analytics.track(eventName)
                }
                
            case .syncToCloud:
                return .run { _ in
                    await cloudSync.sync()
                }
                
            case .profile, .settings, .workout:
                return .none
            }
        }
        
        // Prayer #1: Error Handling
        Prayer(
            listening: [
                \.profile.presented.failedToSave,
                \.workout.presented.failedToSave
            ],
            answerWith: { errorMessage in
                .handleError(errorMessage)
            }
        )
        
        // Prayer #2: Analytics Tracking
        Prayer(
            listening: [
                \.workout.presented.completed,
                \.profile.presented.updated
            ],
            answerWith: { _ in
                .trackAnalytics("user_action")
            }
        )
        
        // Prayer #3: Side Effects Coordination
        Prayer(
            listening: [
                \.profile.presented.updated,
                \.settings.presented.changed
            ],
            answerWith: { _ in
                .syncToCloud
            }
        )
    }
}
